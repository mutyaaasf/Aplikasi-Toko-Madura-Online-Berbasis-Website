...
if(isset($_POST['contact-submit']) && CSRF::validateToken($_POST['token'])) {
    if(isset($_POST['alamat'])) {
        ...
        $statement->execute(array(filter_input(INPUT_POST, 'alamat'), filter_input(INPUT_POST, 'ad-id'. FILTER_SANITIZE_NUMBER_INT)));
    }
    ...
    if(isset($_POST['telepon'])) {
        $statement = $pdo->prepare("UPDATE contact SET name=? WHERE id=?");
        $statement->execute(array(filter_input(INPUT_POST, 'telepon'), filter_input(INPUT_POST, 'ph-id'. FILTER_SANITIZE_NUMBER_INT)));
    }
...
$statement = $pdo->prepare("SELECT * FROM contact WHERE name=?");
$statement->execute(array('telepon'));
$phone = $statement->fetchAll();

$statement = $pdo->prepare("SELECT * FROM contact WHERE name=?");
$statement->execute(array('alamat'));
    ...
    <div class="box box-primary">
        <div class="box-body">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="general-tab" data-bs-toggle="tab" href="#general" role="tab" aria-controls="general" aria-selected="true">Tentang</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="system-tab" data-bs-toggle="tab" href="#system" role="tab" aria-controls="system" aria-selected="false">Kontak</a>
                </li>
                <li class="nav-item">
            ...
            <div class="tab-content" id="myTabContent">
                    ...
                    <div class="col-md-6">
                        <form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
                            ...
                            <div class="mb-3 text-end">
                                <button name="about-submit" class="btn btn-success" type="submit"><i class="fas fa-check"></i> Perbarui</button>
                            </div>
                        </form>
                    </div>
                </div>
                        ...
                        <form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
                            <div class="mb-3">
                                <div class="input-group mb-3">
                                    ...
                                    <input type="text" name="alamat" class="form-control" value="<?= $address[0]['value'] ?>">
                            ...
                            <div class="mb-3">
                                <div class="input-group mb-3">
                                    ...
                                    <input type="tel" name="telepon" class="form-control" value="<?= $phone[0]['value'] ?>">
                            ...
                            <div class="mb-3 text-end">
                                <button class="btn btn-success" name="contact-submit" type="submit"><i class="fas fa-check"></i> Perbarui</button>
                            </div>
                        </form>
                    </div>
                </div>
                <div class="tab-pane fade" id="attributions" role="tabpanel" aria-labelledby="attributions-tab">
                    ...
                    <p class="text-muted">Copyright (c) <script>document.write(new Date().getFullYear());</script> TomadOn. All rights reserved.</p>
                    <form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post">
                        <?php CSRF::csrfInputField() ?>
                        <textarea class="form-control" name="policy" style="resize:none" required rows="20"><?= $privacyPolicy[0]['policy'] ?></textarea>            
                        <div class="mb-3 text-end">
                            <button class="btn btn-success" name="policy-submit" type="submit"><i class="fas fa-check"></i> Perbarui</button>
                        </div>
                    </form>
                </div>
            </div>
